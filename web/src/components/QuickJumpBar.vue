<template>
  <div class="w-full my-3 flex flex-row justify-center items-center">
    ...
    <select
      ...
      <option v-for="book in group_books.BookInfos" :value="book.id" :key="book.id" :selected="book.id == nowBookID">
        {{ book.title }}
      </option>
    </select>
...
export default defineComponent({
  ...
  computed: {
    prevLink() {
      for (let i = 0; i < this.group_books.BookInfos.length; i++) {
        if (this.group_books.BookInfos[i].id === this.nowBookID) {
          if (i === 0) {
            return `/#/scroll/${this.nowBookID}`;
          }
          return `/#/scroll/${this.group_books.BookInfos[i - 1].id}`;
        }
      }
    },
    nextLink() {
      for (let i = 0; i < this.group_books.BookInfos.length; i++) {
        if (this.group_books.BookInfos[i].id === this.nowBookID) {
          if (i === this.group_books.BookInfos.length - 1) {
            return `/#/scroll/${this.nowBookID}`;
          }
          return `/#/scroll/${this.group_books.BookInfos[i + 1].id}`;
        }
  ...
  methods: {
    async fetchQuickJumpInfo() {
      try {
        const response = await axios.get(`/group_books?id=${this.$route.params.id}`);
        this.group_books = response.data;
      } catch (error) {
        console.log(error);
      }
    }
