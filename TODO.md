...
- 当前版本线：`v1.2.31`。
- 已具备：Web 书架、卷轴阅读、翻页阅读、本地 `/reader`、便携 HTML、PWA、Tailscale、账号/权限、阅读历史、书签、上传到书库、服务器设置页、日志页、OPDS、TUI 预览、Windows 托盘/文件关联、SQLite 兼容层、SMB/WebDAV 等 VFS 基础能力。
- 近期已完成：上传入口迁移到设置页、OPDS 路由与测试、书架最近阅读排序文案修正、只读设置页可读不可改、本地 reader 的便携 HTML 与非 loopback 访问兼容性修复。

## 优先处理

### P0：真实环境回归

- [ ] 复查 `/reader` 在 ZeroTier/Tailscale/局域网 IP 下打开本地压缩包的问题。要求用真实浏览器文件选择或等价自动化覆盖 `http://192.168.*`、`http://100.*`、`localhost/127.0.0.1`，确认 `FileReader` 兜底路径是否真正命中。
- [ ] 复查便携 HTML `file://` 打开场景：不得请求 `/api/reading-history`，不得注册 Service Worker，不得加载根路径 favicon，WASM/reader 静态资源必须可用。
- [ ] 把浏览器审计的手工步骤固化成最小 smoke 测试脚本，覆盖首页、书架、scroll、flip、player、settings、reader、静态导出页面。

### P1：安全与稳定性

- [ ] 统一前端文本写入边界。优先处理 `templ/common/toast.templ`、`templ/pages/upload_page/upload_area.templ`、`templ/pages/settings/log_panel.templ`，避免把日志、toast message、上传文件名直接拼进 `innerHTML`。
- [ ] 为图片生成和图片变换参数增加统一上下限。覆盖 `routers/data_api/get_generated_image.go`、`routers/data_api/get_file.go`，非法宽高、字体大小、文本长度、blurhash 参数应返回 400。
- [ ] 把普通读接口里的全局清理副作用移出请求路径。`GetBook`、首页书架、子书架不应在普通 GET 中触发 `ClearBookWhenStoreUrlNotExist` / `ClearBookNotExist`。
- [ ] 扫描流程去掉包级全局状态。`tools/scan` 中的 `cfg/currentFS` 应收敛为显式扫描上下文，短期至少串行化扫描入口，避免上传扫描和手动重扫互相覆盖。
...
- [ ] `sample/wiki/03-Try-it-Online.md`：补充“选择本地压缩包后直接阅读”的截图或短动图。
- [ ] `sample/wiki/03-Try-it-Online.md`：根据最新 `/reader` 行为更新离线/PWA/便携 HTML 说明，特别说明 `file://` 便携 HTML 不连接服务器。
- [ ] `sample/wiki/08-Reading-and-Library.md`：补充本地 reader、在线 reader、服务器书库三者差异，以及适合的文件大小/内存限制说明。
- [ ] `sample/wiki/09-Remote-and-Security.md`：补充 Tailscale、ZeroTier、局域网 IP、只读模式、上传开关和文件不会上传到在线 reader 的边界说明。
- [ ] `sample/wiki/11-Changelog-ZH.md`、`12-Changelog-JA.md`、`13-Changelog-EN.md`：同步 `v1.2.31` 之后的变更，包含 OPDS、TUI、reader/PWA、设置页只读体验和日志页优化。
- [ ] `README.md`、`README_ZH.md`、`README_JP.md`：同步 Wiki 中已经稳定的安装、PWA、OPDS、TUI 和 reader 说明。
...
- [ ] 内存 Store 与 SQLite Store 的书组生成逻辑抽成无副作用服务，避免两套存储实现行为漂移。
- [ ] SQLite 如果进入主路径，需要减少 `ListBooks` 的 N+1 查询，补批量查询和大书库性能测试。
