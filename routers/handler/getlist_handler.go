...
func GetBookListHandler(c *gin.Context) {
	...
	//如果传了maxDepth这个参数
	if err == nil {
		bookInfoList, err := book.GetBookInfoListByMaxDepth(maxDepth, sort_by)
		if err != nil {
			fmt.Println(err)
			return
		}
		bookInfoList.SortBooks(sort_by)
	...
	//如果传了bookGroupId这个参数
	if bookGroupId != "" {
		bookInfoList, err := book.GetBookInfoListByBookGroupBookID(bookGroupId, sort_by)
		if err != nil {
			fmt.Println(err)
		}
		bookInfoList.SortBooks(sort_by)
	...
	//如果传了depth这个参数
	if err == nil {
		bookInfoList, err := book.GetBookInfoListByDepth(depth, sort_by)
		if err != nil {
			fmt.Println(err)
		} else {
			bookInfoList.SortBooks(sort_by)
