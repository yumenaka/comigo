...
func SaveConfigHandler(c *gin.Context) {
	...
	if SaveTo == "HomeDirectory" && (config.Status.Path.WorkingDirectory != "" || config.Status.Path.ProgramDirectory != "") {
		logger.Info("error: Find config in " + config.Status.Path.WorkingDirectory + " " + config.Status.Path.ProgramDirectory)
	...
	if SaveTo == "ProgramDirectory" && (config.Status.Path.WorkingDirectory != "" || config.Status.Path.HomeDirectory != "") {
		logger.Info("error: Find config in " + config.Status.Path.WorkingDirectory + " " + config.Status.Path.HomeDirectory)
