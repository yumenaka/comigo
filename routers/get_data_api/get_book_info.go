...
func GetParentBook(c echo.Context) error {
	...
	book, err := model.MainStores.GetParentBook(id)
	if err != nil {
		logger.Infof("%s", err)
...
func GetTopOfShelfInfo(c echo.Context) error {
	...
	bookInfoList, err := model.MainStores.TopOfShelfInfo(sortBy)
	if err != nil {
		logger.Infof("%s", err)
...
func GroupInfo(c echo.Context) error {
	...
	infoList, err := model.MainStores.GetBookInfoListByParentFolder(b.ParentFolder, sortBy)
	if err != nil {
		logger.Infof("%s", err)
