...
func updateJSONStorage(newRoot DirNode) error {
	// 用互斥锁保护数据更新
	dataMutex.Lock()
	rootData = newRoot // 更新全局的内存数据为最新的扫描结果
	dataMutex.Unlock()
...
func loadJSONStorage() error {
	dataBytes, err := ioutil.ReadFile("scan_results.json")
	if err != nil {
		return err
	}
...
func updateSQLiteStorage(foundDirs []string, foundFiles []ImageInfo) error {
	// 检查entClient是否已初始化
	if entClient == nil {
	...
	// 预处理图片路径
	validImages := make([]ImageInfo, 0, len(foundFiles))
	for _, img := range foundFiles {
		// 规范化路径
		normalizedPath := normalizePath(img.Path)
		if isValidPath(normalizedPath) {
	...
	for _, img := range validImages {
		...
		parentDirPath = normalizePath(parentDirPath)
