...
func (iq *ImageQuery) QueryDirectory() *DirectoryQuery {
	query := (&DirectoryClient{config: iq.config}).Query()
...
func (iq *ImageQuery) Clone() *ImageQuery {
	...
	return &ImageQuery{
		...
		withDirectory: iq.withDirectory.Clone(),
...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	var (
		...
		_spec       = iq.querySpec()
		loadedTypes = [1]bool{
			iq.withDirectory != nil,
		}
	)
	if iq.withDirectory != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := iq.withDirectory; query != nil {
		if err := iq.loadDirectory(ctx, query, nodes, nil,
			func(n *Image, e *Directory) { n.Edges.Directory = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadDirectory(ctx context.Context, query *DirectoryQuery, nodes []*Image, init func(*Image), assign func(*Image, *Directory)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Image)
	for i := range nodes {
		if nodes[i].directory_images == nil {
			continue
		}
		fk := *nodes[i].directory_images
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "directory_images" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
