...
- Kitty：预览区正常；终端阅读回到 Unicode placeholder 路径，让图片随 Bubble Tea 文本行刷新，避免页码和图像层不同步。
- Ghostty：支持 Kitty graphics protocol；终端阅读回到 Unicode placeholder 路径。预览区不走 placeholder，改用 Kitty overlay，避免裁切或尺寸过小。
...
| Sixel | 传统 DEC bitmap 图形协议，需要把图片编码为 sixel 数据流。 | xterm/foot/mlterm/Windows Terminal/WezTerm 等生态有支持，但颜色、性能、tmux、启用状态差异大。WezTerm 官方标为 experimental。 | 目前仅作为可强制实验协议，不作为默认路径。 |
