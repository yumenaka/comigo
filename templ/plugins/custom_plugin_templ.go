...
func RenderCustomPlugins(c echo.Context) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		if config.GetCfg().EnablePlugin {
			...
			if strings.HasPrefix(customPluginRequestPath(c), "/shelf") || customPluginRequestPath(c) == "/" {
			...
			if strings.HasPrefix(customPluginRequestPath(c), "/flip") {
				...
				if bookID := extractBookID(customPluginRequestPath(c)); bookID != "" {
			...
			if strings.HasPrefix(customPluginRequestPath(c), "/scroll") {
...
func customPluginRequestPath(c echo.Context) string {
	if c == nil || c.Request() == nil || c.Request().URL == nil {
		return "/"
	}
	return config.StripBasePath(c.Request().URL.Path)
}
