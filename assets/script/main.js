...
class $1bac384020b50752$var$I18n extends $1bac384020b50752$var$EventEmitter {
    ...
    cloneInstance(options = {}, callback = $1bac384020b50752$var$noop) {
        ...
        if (options.interpolation) clone.services.interpolator = new $1bac384020b50752$var$Interpolator(mergedOptions);
...
Alpine.store('flip', {
    ...
    // ============ 滑动动画配置（新增）============
    // 滑动动画持续时间（毫秒）
    swipeAnimationDuration: Alpine.$persist(200).as('flip.swipeAnimationDuration'),
    // 回弹动画持续时间（毫秒）
    resetAnimationDuration: Alpine.$persist(100).as('flip.resetAnimationDuration'),
    // 滑动阈值（像素）- 超过这个值才会触发翻页
    swipeThreshold: Alpine.$persist(100).as('flip.swipeThreshold'),
    // 快速滑动超时时间（毫秒）
    swipeTimeout: Alpine.$persist(200).as('flip.swipeTimeout'),
    // ============ 其他可配置参数（新增）============
    // 预加载图片范围
