...
func Smb(scanOption Option) (newBookList []*types.Book, err error) {
	...
	session, err := dialer.Dial(context.Background(), scanOption.RemoteStores[0].Host+":"+strconv.Itoa(scanOption.RemoteStores[0].Port))
	if err != nil {
		panic(err)
	}
	...
	//		fmt.Println("smb hint： smb://"+scanOption.RemoteStores[0].Host+"/"+scanOption.RemoteStores[0].ShareName+"/test/"+path, d.IsDir(), err)
	//		return nil
	//	})
	//if err != nil {
	//	panic(err)
	...
	err = iofs.WalkDir(
		...
		func(walkPath string, fileInfo iofs.DirEntry, err error) error {
			...
			if !fileInfo.IsDir() {
				...
				// 得到书籍文件数据
				getBook, err := smbScanFile(walkPath, file, scanOption.RemoteStores[0].ShareName, depth, scanOption)
				if err != nil {
			...
			//if fileInfo.IsDir() {
			//	// 得到书籍文件数据
			//	getBook, err := smbScanDir(walkPath, scanOption.RemoteStores[0].ShareName, depth, scanOption)
			//	if err != nil {
			//		logger.Infof("%e", err)
			//		return nil
			//	}
	...
	return nil, errors.New("NO_BOOKS_FOUND in SMB:" + scanOption.RemoteStores[0].ShareName)
}

// 扫描本地路径，并返回对应书籍
func smbScanFile(filePath string, file *smb2.File, storePath string, depth int, scanOption Option) (newBook *types.Book, err error) {
...
func smbScanDir(dirPath string, storePath string, depth int, scanOption Option) (*types.Book, error) {
	// 初始化，生成UUID
