...
const StringArrayInput: React.FC<Props> = ({
    ...
    return (
        <div
            className="flex flex-row my-2 w-2/3 font-semibold rounded-md shadow-md bg-yellow-300 justify-start items-center">
            <label htmlFor={name} className="w-32  mx-8 my-2 border border-black rounded-md">
                {label}:
            </label>
            <div className="flex flex-row flex-wrap mx-4 py-1">
                {/* {value.toString()} */}
                ...
                <input className="my-1 px-2 border border-black rounded-md" type="text" placeholder="Add new " onKeyDown={(e) => {
                    if (e.key === 'Enter') {
