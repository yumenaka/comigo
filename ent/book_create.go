...
func (bc *BookCreate) createSpec() (*Book, *sqlgraph.CreateSpec) {
	...
	if value, ok := bc.mutation.BookStorePath(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: book.FieldBookStorePath,
		})
