...
func (bq *BookQuery) QueryPageInfos() *SinglePageInfoQuery {
	query := (&SinglePageInfoClient{config: bq.config}).Query()
...
func (bq *BookQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Book, error) {
	...
	if query := bq.withPageInfos; query != nil {
		if err := bq.loadPageInfos(ctx, query, nodes,
			func(n *Book) { n.Edges.PageInfos = []*SinglePageInfo{} },
			func(n *Book, e *SinglePageInfo) { n.Edges.PageInfos = append(n.Edges.PageInfos, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BookQuery) loadPageInfos(ctx context.Context, query *SinglePageInfoQuery, nodes []*Book, init func(*Book), assign func(*Book, *SinglePageInfo)) error {
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "book_page_infos" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
