...
- `mobile/` - 面向宿主端的导出包，供 `gomobile bind` 生成 Android AAR / iOS / macOS XCFramework
...
- `assets/script/` 中，`assets/script/styles.css` 与 `assets/script/main.js` 是 `bun run dev` 编译生成的文件。
- `assets/script/` 也有一些文件静态是导入，开发时可以直接修改源文件。文件静态导入的方式是通过 `common.Html()` 的第三个参数 `insertScripts` 来插入到生成的 HTML 中的，如下所示：
...
- 嵌入式宿主：Android / iOS / macOS 侧通过 `gomobile bind` 将 `mobile/` 打包为原生库，由宿主 App 启动本地 HTTP 服务
...
- 嵌入式 iOS 构建：由主仓库执行 `scripts/build_ios_go.sh`，实际对 `cvgo/mobile` 运行 `gomobile bind`
- 嵌入式 macOS 构建：由主仓库执行 `scripts/build_macos_go.sh`，实际对 `cvgo/mobile` 运行 `gomobile bind`
- 开发时假设 CLI 工具与依赖已安装
