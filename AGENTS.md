...
- templ/下面的 *_templ.go文件是templ生成的，不要手动修改。如果需要修改，请修改对应的*.templ文件，然后执行`go run github.com/a-h/templ/cmd/templ@latest fmt ./templ && go run github.com/a-h/templ/cmd/templ@latest generate`命令生成。
...
- 运行指令：go run github.com/a-h/templ/cmd/templ@latest fmt ./templ && go run github.com/a-h/templ/cmd/templ@latest generate && go run main.go